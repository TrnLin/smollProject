...
const Navbar = () => {
  ...
  return (
    <nav className='w-[calc(100%-2rem)] top-4 left-1/2 -translate-x-1/2 fixed right-0 z-50 '>
      <div className='flex justify-between ring-1 ring-neutral-100 items-center bg-neutral-0  px-3 relative py-4 rounded max-h-11'>
        <div>
          <Link to='/'>
            <svg
              width='24'
        ...
        <div className='flex flex-row '>
          <HamergerMenuToggle />
          <ul className=' space-y-2  absolute w-full top-[calc(100%+12px)] rounded ring-1 ring-neutral-100 left-0  p-4  '>
...
const NavItem: React.FC<NavItemProps> = ({ href, children }) => {
  return (
    <li className='text-base text-neutral-1000 w-full'>
      <NavLink className={` block  py-1 w-full`} to={href}>
        {children}
      </NavLink>
    </li>
  );
...
const HamergerMenuToggle = () => {
  return <span onClick={()} className='block sm:hidden text-neutral-1000'>Menu</span>;
};
